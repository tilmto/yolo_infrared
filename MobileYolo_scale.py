...
class MobileYolo_scale:
	def __init__(self,images,labels,anchors,best_scale,is_training=True,img_size=512,scope='Yolo_MobileNet_v2'):
		self.images = images
		self.labels = labels
		...
		self.loss_scale = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.scale_pred,labels=tf.cast(self.best_scale,tf.int32)))
		self.optimizer_scale = tf.train.AdamOptimizer(1e-4).minimize(self.loss_scale,var_list=[v for v in tf.trainable_variables() if v.name.find('scale')!=-1])

		self.optimizer_global = tf.train.AdamOptimizer(1e-4).minimize(self.loss+self.loss_scale)
	...
	def build_model(self,scope,is_training=True):
		...
		with tf.variable_scope(scope):
			...
			x = self.residual(x,out_channels=320,multi=6,stride=2,is_training=self.is_training,scope='residual_320_1')

			x = slim.conv2d(x,1280,[1,1],2,padding='SAME',activation_fn=None,scope='conv2')
			x = tf.nn.relu6(self.batch_norm(x,is_training,scope='bn2'))

			x = self.global_avg(x)

			x = slim.fully_connected(x,int(16*16*self.anchors_per_unit*8),scope='fc_pred',activation_fn=None)
			self.scale_pred = slim.fully_connected(x,6,scope='fc_scale',activation_fn=None)

			self.model_output = tf.reshape(x,[-1,16*16,self.anchors_per_unit,8],name='model_output')
...
if __name__ == '__main__':
	...
	anchors = np.ones([5,2],dtype=np.float32)

	with tf.Session() as sess:
		model = MobileYolo_scale(images,labels,anchors,best_scale_plh)
		sess.run(tf.global_variables_initializer())
		print(model.predict.shape.as_list())
		print('Size:',model_size())
		var_list=[v for v in tf.trainable_variables() if v.name.find('scale')!=-1]
		print(var_list)