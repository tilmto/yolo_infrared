...
class MobileYolo_dark:
	def __init__(self,images,labels,anchors,is_training=True,img_size=512,scope='Yolo_MobileNet_v2_dark'):
		self.images = images
		self.labels = labels
	...
	def build_model(self,scope,is_training=True):
		...
		with tf.variable_scope(scope):
			...
			branch1 = slim.conv2d(x,self.final_channels,[4,4],4,padding='SAME',activation_fn=None,scope='branch1')
			...
			branch2 = slim.conv2d(x,self.final_channels,[2,2],2,padding='SAME',activation_fn=None,scope='branch2')
			...
			x = tf.nn.relu6(self.batch_norm(x + branch1 + branch2,is_training,scope='bn3'))

			x = slim.conv2d(x,self.final_channels,[1,1],1,padding='SAME',activation_fn=None,scope='conv4')
...
if __name__ == '__main__':
	...
	with tf.Session() as sess:
		model = MobileYolo_dark(images,labels,anchors)
